...
parser = PaymentsDive(driver(), max_count_documents=10)
...
try:
    ...
    print(f'error with backup in main: {e}')

try:
    dataframe = pandas.DataFrame.from_records([to_dict(d) for d in docs])
    dataframe.to_csv('out/paymentsdive_documents.csv')
except Exception as e:
    print(f'error with backup in export to df: {e}')
